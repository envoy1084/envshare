...
pub fn write_private_atomic(
    ...
    #[cfg(windows)]
    let private_directory = create_private_staging_directory(&canonical_parent)?;
    #[cfg(windows)]
    let temporary_parent = private_directory.path();
    #[cfg(not(windows))]
    let temporary_parent = canonical_parent.as_path();
    let mut temporary =
        tempfile::NamedTempFile::new_in(temporary_parent).map_err(|_| CoreError::Output)?;
    make_private(temporary.as_file(), temporary.path()).map_err(|_| CoreError::Output)?;
    temporary
        .write_all(payload)
...
#[cfg(unix)]
fn make_private(file: &File, _path: &Path) -> std::io::Result<()> {
...
#[cfg(windows)]
fn make_private(_file: &File, path: &Path) -> std::io::Result<()> {
    set_private_dacl(path, false)
}

#[cfg(windows)]
fn create_private_staging_directory(parent: &Path) -> Result<tempfile::TempDir, CoreError> {
    let directory = tempfile::Builder::new()
        .prefix(".envshare-")
        .tempdir_in(parent)
        .map_err(|_| CoreError::Output)?;
    set_private_dacl(directory.path(), true).map_err(|_| CoreError::Output)?;
    Ok(directory)
}

#[cfg(windows)]
fn set_private_dacl(path: &Path, inheritable: bool) -> std::io::Result<()> {
    ...
    let descriptor = format!("D:P(A;{inheritance};FA;;;{current_user})")
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn windows_output_grants_only_current_user_full_control() -> Result<(), CoreError> {
        use windows_permissions::{
        ...
        .map_err(|_| CoreError::Output)?;
        let dacl = descriptor.dacl().ok_or(CoreError::Output)?;
        let ace = dacl.get_ace(0).ok_or(CoreError::Output)?;
        let current_user =
            windows_permissions::utilities::current_process_sid().map_err(|_| CoreError::Output)?;
        assert_eq!(dacl.len(), 1);
        assert_eq!(ace.sid(), Some(current_user.as_ref()));
