...
| Normal output, logs, metrics, and errors exclude secrets | Verified | Sentinel CLI failure tests, redacted `Debug` tests, identifier-free node JSON tests, low-cardinality metric tests, and the [secret-output policy](secret-output-policy.md). This does not substitute for independent review. |
