...
repeated `--discovery-node` values. `--mdns` enables local discovery, `--lan`
admits private candidate routes, and `--relay-only` excludes direct routes and
mDNS. The sender does not reveal the capability until a configured public route
has accepted its registration.
...
required relay reservations. It never derives diagnostics from a user
capability. `--json` emits bounded counts without namespaces or payload data.
