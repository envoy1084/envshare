# Documentation

Envshare is pre-release software and is not yet approved for production secrets.
Start with the [user guide](user-guide.md), then use the focused references below.

## Use Envshare

- [Installation](installation.md)
- [User guide](user-guide.md)
- [Command-line reference](cli.md)
- [Client configuration](configuration.md)
- [Troubleshooting](troubleshooting.md)

## Understand the protocol and security boundary

- [Architecture](architecture.md)
- [Protocol specification](../protocol/protocol.md)
- [Protocol compatibility](protocol-compatibility.md)
- [Capability format](../protocol/code-format.md)
- [Threat model](threat-model.md)
- [Security guide](security.md)
- [Privacy and data handling](privacy.md)
- [Secret output policy](secret-output-policy.md)
- [Secret output audit](secret-output-audit.md)
- [Acceptance evidence](acceptance-evidence.md)

## Self-host and operate nodes

- [Node configuration](node-configuration.md)
- [Node identity](node-identity.md)
- [Deployment](deployment.md)
- [Operations runbook](operations.md)
- [Load and soak testing](load-testing.md)

## Ship releases

- [Release and rollback](release.md)
- [Release quality gates](quality-gates.md)
- [Security reporting policy](../SECURITY.md)
- [Contribution workflow](../CONTRIBUTING.md)
